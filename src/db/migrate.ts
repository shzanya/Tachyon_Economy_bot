  ...
  try {
    ...
    try {
      const { rows } = await client.query(`
        ...
        AND table_name IN ('users', 'transactions', 'user_activity', 'shop_roles', 'user_roles', 'user_rooms', 'user_purchases', 'user_keys')
        ORDER BY table_name
      `);
      
      if (rows.length > 0) {
        logger.info(`📊 Обнаружены таблицы: ${rows.map((r: any) => r.table_name).join(', ')}`);
      } else {
