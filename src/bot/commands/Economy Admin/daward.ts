...
const data = new SlashCommandBuilder()
  .setName('daward')
  .setDescription('👑 [Админ] Выдать донатную валюту пользователям.')
...
async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    await interaction.editReply({ embeds: [AdminEmbeds.success(admin, uniqueRecipients, amount, 'diamond', 'award')] });
    logger.info(`👑 DAWARD: ${admin.tag} awarded ${amount} diamonds to ${uniqueRecipients.length} users.`);
  } catch (error) {
    logger.error('Daward command error:', error);
    await interaction.editReply({ embeds: [AdminEmbeds.error('Произошла ошибка при начислении валюты.')] });
  }
}

export const dawardCommand: BotCommand = { data, execute };
