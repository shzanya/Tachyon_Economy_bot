...
const data = new SlashCommandBuilder()
  .setName('take')
  .setDescription('👑 [Админ] Забрать обычную валюту у пользователей.')
...
async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    const transactions = uniqueRecipients.map(user => {
      const finalReason = `Списание администратором: ${reason}`;
    ...
    await interaction.editReply({ embeds: [AdminEmbeds.success(admin, uniqueRecipients, amount, 'coin', 'take')] });
    logger.info(`👑 TAKE: ${admin.tag} took ${amount} coins from ${uniqueRecipients.length} users.`);
  } catch (error) {
    logger.error('Take command error:', error);
...
export const takeCommand: BotCommand = { data, execute };
